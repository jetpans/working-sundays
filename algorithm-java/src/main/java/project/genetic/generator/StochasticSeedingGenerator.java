...
public class StochasticSeedingGenerator extends ForStoresGenerator {
     ...
     */
    public StochasticSeedingGenerator(List<String> storeIds, List<MatrixChromosome> seeds, double p, Generator<MatrixChromosome> fallbackGenerator) {
        super(storeIds);
    ...
    public MatrixChromosome generate() {
        ...
        // Stochastic setting of some models
        for (int i = 0; i < storeIds.size(); i++) {
            if (Global.RANDOM.nextDouble() > p) {
                continue; // Skip randomization for this store
            }
            List<Integer> allStores = (new ArrayList<>(child.getModel(i)));
            allStores.addAll(child.getAntiModel(i));
            List<Integer> newModel = new ArrayList<>();
            while (newModel.size() < child.getModel(i).size()) {
                int randomIndex = Global.RANDOM.nextInt(allStores.size());
                newModel.add(allStores.get(randomIndex));
                allStores.remove(randomIndex);
            }
            child.setModel(i, newModel);
        }
